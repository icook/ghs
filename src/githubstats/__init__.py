...
def update_metrics(path):
    from githubstats.serve import mongo
    ...
    mongo.db.packages.insert({"name": "yota"})
    for root, dirnames, filenames in os.walk(path):
        for filename in fnmatch.filter(filenames, '*.py'):
            ...
            try:
                ...
                mongo.db.packages.update({"name": "yota"},
                                         {"$push": {"files":
                                                            {"filename": f,
                                                            "characters": metrics['numCharacters'],
                                                            "comments": metrics['numComments'],
                                                            "func_count": metrics['numFunctions']
                                                            }
                                                        }
                                                        })
